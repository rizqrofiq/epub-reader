...
function AuthForm() {
  ...
  return (
    <div className="flex min-h-screen">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-bg-secondary items-center justify-center">
        <div className="absolute inset-0">
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-accent/10 rounded-full blur-3xl animate-pulse-ring" />
          <div className="absolute bottom-1/3 right-1/4 w-80 h-80 bg-accent/5 rounded-full blur-3xl animate-pulse-ring [animation-delay:1s]" />
        </div>
        ...
        <div className="relative z-10 text-center px-12">
          ...
          <p className="text-xl text-text-secondary max-w-md mx-auto leading-relaxed">
            Your personal EPUB reader with cloud-synced progress, highlights,
            and bookmarks.
          </p>

          <div className="mt-10 flex flex-wrap gap-3 justify-center">
            {[
              { icon: "cloud_sync", text: "Cloud Sync" },
              { icon: "ink_highlighter", text: "Highlights" },
              { icon: "bookmark", text: "Bookmarks" },
              { icon: "add_to_drive", text: "Google Drive" },
            ].map((f) => (
              <div
                key={f.text}
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-bg-elevated/80 border border-border text-sm text-text-secondary"
              >
                <span className="material-symbols-rounded sm">{f.icon}</span>
                {f.text}
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-bg-primary">
        <div className="w-full max-w-md">
          <div className="lg:hidden text-center mb-8">
            ...
            <h1 className="text-3xl font-bold">Readium</h1>
          </div>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="bg-bg-primary px-4 text-text-tertiary">
            ...
            >
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  <span className="material-symbols-rounded sm animate-spin">
                    progress_activity
                  </span>
